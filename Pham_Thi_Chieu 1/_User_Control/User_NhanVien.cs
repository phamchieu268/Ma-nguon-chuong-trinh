...
namespace Pham_Thi_Chieu._User_Control
{
    public partial class User_NhanVien : UserControl
    {
        ...
        private void User_NhanVien_Load(object sender, EventArgs e)
        {
            ...
            // đây là combobox dislayMember là file hiển thị trên combobox đó
            // valuemember là giá trị của comboboxx đo
        ...
        {
            string sql_seatch_ten = "select *  from NhanVien where Ten_NV like N'%"+txt_Search_Ten.Text.ToString()+"%'";
           dgvNhanVien.DataSource = db.Excute(sql_seatch_ten);
            
        }
        #endregion

        private void txt_Search_SoCMND_TextChanged(object sender, EventArgs e)
        {
            string sql_seatch_socmnd = "select *  from NhanVien where So_CMND like N'%" + txt_Search_SoCMND.Text.ToString() + "%'";
            dgvNhanVien.DataSource = db.Excute(sql_seatch_socmnd);
        }
